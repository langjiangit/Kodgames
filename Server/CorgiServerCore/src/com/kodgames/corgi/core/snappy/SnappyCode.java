...
/**
 * Uncompresses an input {@link ByteBuf} encoded with Snappy compression into an output {@link ByteBuf}.
 *
 * See http://code.google.com/p/snappy/source/browse/trunk/format_description.txt
 */
public class SnappyCode
{
	...
	public void encode(MyByteBuffer in, MyByteBuffer out, int length)
	{
		try
		{
		...
		final int[] table = getHashTable(length);
		...
		if (length - inIndex >= MIN_COMPRESSIBLE_BYTES)
		{
			...
			outer: while (true)
			{
				...
				do
				{
					...
					int tmp = inIndex - baseIndex;
					if (tmp < 0)
					{
						System.out.println(String.format("inIndex - baseIndex == %d -- %d", inIndex, baseIndex));
					}
					table[hash] = inIndex - baseIndex;
				...
				do
				{
					...
					table[prevHash] = (inIndex - baseIndex - 1);
					int tmp = inIndex - baseIndex - 1;
					if (tmp < 0)
					{
						System.out.println(String.format("inIndex - baseIndex - 1 == %d -- %d", inIndex, baseIndex));
					}
					int currentHash = hash(in, insertTail + 1, shift);
					candidate = baseIndex + table[currentHash];
					tmp = inIndex - baseIndex;
					if (tmp < 0)
					{
						System.out.println(String.format("inIndex - baseIndex == %d -- %d", inIndex, baseIndex));
					}
					table[currentHash] = (inIndex - baseIndex);
	 ...
	 * @param shift The shift value, for ensuring that the resulting value is withing the range of our hash table size
	 * @return A 32-bit hash of 4 bytes located at index
	 */
	private static int hash(MyByteBuffer in, int index, int shift)
	{
	...
	/**
	 * Iterates over the supplied input buffer between the supplied minIndex and maxIndex to find how long our matched
	 * copy overlaps with an already-written literal value.
	 *
	 ...
	 */
	private static int findMatchingLength(MyByteBuffer in, int minIndex, int inIndex, int maxIndex)
	{
		int matched = 0;
	...
	/**
	 * Calculates the minimum number of bits required to encode a value. This can then in turn be used to calculate the
	 * number of septets or octets (as appropriate) to use to encode a length parameter.
	 *
	...
	/**
	 * Writes a literal to the supplied output buffer by directly copying from the input buffer. The literal is taken
	 * from the current readerIndex up to the supplied length.
	 *
	 ...
	 */
	private static void encodeLiteral(MyByteBuffer in, MyByteBuffer out, int length)
	{
		if (length < 61)
		{
	...
	private static void encodeCopyWithOffset(MyByteBuffer out, int offset, int length)
	{
	 ...
	 */
	private static void encodeCopy(MyByteBuffer out, int offset, int length)
	{
		while (length >= 68)
		{
	...
	public void decode(MyByteBuffer in, MyByteBuffer out)
	{
	...
	/**
	 * Reads the length varint (a series of bytes, where the lower 7 bits are data and the upper bit is a flag to
	 * indicate more bytes to be read).
	 *
	 * @param in The input buffer to read the preamble from
	 * @return The calculated length based on the input buffer, or 0 if no preamble is able to be calculated
	 */
	private static int readPreamble(MyByteBuffer in)
	{
		int length = 0;
		int byteIndex = 0;
		while (in.isReadable())
	...
	/**
	 * Reads a literal from the input buffer directly to the output buffer. A "literal" is an uncompressed segment of
	 * data stored directly in the byte stream.
	 *
	 * @param tag The tag that identified this segment as a literal is also used to encode part of the length of the
	 *            data
	 ...
	 */
	private static int decodeLiteral(byte tag, MyByteBuffer in, MyByteBuffer out)
	{
		in.markReaderIndex();
		int length;
		switch (tag >> 2 & 0x3F)
		{
			...
			case 61:
				...
				length = BufferUtil.swapShort(in.readShort());
				break;
			case 62:
				...
				length = BufferUtil.swapMedium(in.readUnsignedMedium());
				break;
			case 64:
				...
				length = BufferUtil.swapInt(in.readInt());
				break;
			default:
				length = tag >> 2 & 0x3F;
		}
	...
	/**
	 * Reads a compressed reference offset and length from the supplied input buffer, seeks back to the appropriate
	 * place in the input buffer and writes the found data to the supplied output stream.
	 *
	 * @param tag The tag used to identify this as a copy is also used to encode the length and part of the offset
	 * @param in The input buffer to read from
	 * @param out The output buffer to write to
	 * @return The number of bytes appended to the output buffer, or -1 to indicate "try again later"
	 * @throws DecompressionException If the read offset is invalid
	 */
	private static int decodeCopyWith1ByteOffset(byte tag, MyByteBuffer in, MyByteBuffer out, int writtenSoFar)
	{
		if (!in.isReadable())
		{
			return NOT_ENOUGH_INPUT;
	 ...
	 * @return The number of bytes appended to the output buffer, or -1 to indicate "try again later"
	 */
	private static int decodeCopyWith2ByteOffset(byte tag, MyByteBuffer in, MyByteBuffer out, int writtenSoFar)
	{
		if (in.readableBytes() < 2)
		{
			return NOT_ENOUGH_INPUT;
		...
		int offset = BufferUtil.swapShort(in.readShort());
	 ...
	 * @throws DecompressionException If the read offset is invalid
	 */
	private static int decodeCopyWith4ByteOffset(byte tag, MyByteBuffer in, MyByteBuffer out, int writtenSoFar)
	{
		if (in.readableBytes() < 4)
		{
			return NOT_ENOUGH_INPUT;
		...
		int initialIndex = out.writerIndex();
		int length = 1 + (tag >> 2 & 0x03F);
		int offset = BufferUtil.swapInt(in.readInt());
	...
	/**
	 * Validates that the offset extracted from a compressed reference is within the permissible bounds of an offset (4
	 * <= offset <= 32768), and does not exceed the length of the chunk currently read so far.
	 *
	...
	private static void validateOffset(int offset, int chunkSizeSoFar)
	{
		...
		{
			throw new IllegalArgumentException("Offset exceeds maximum permissible value");
		}

		if (offset <= 0)
		{
			throw new IllegalArgumentException(String.format("Offset %d is less than minimum permissible value", offset));
		}

		if (offset > chunkSizeSoFar)
		{
			throw new IllegalArgumentException(String.format("Offset %d exceeds size of chunk %d", offset, chunkSizeSoFar));
		}
	}
	 ...
	 */
	public static int calculateChecksum(MyByteBuffer data)
	{
	 ...
	 * @param data The input data to calculate the CRC32C checksum of
	 */
	public static int calculateChecksum(MyByteBuffer data, int offset, int length)
	{
	...
	/**
	 * Computes the CRC32C checksum of the supplied data, performs the "mask" operation on the computed checksum, and
	 * then compares the resulting masked checksum to the supplied checksum.
	 *
	 ...
	 */
	static void validateChecksum(int expectedChecksum, MyByteBuffer data)
	{
	 ...
	 * @throws DecompressionException If the calculated and supplied checksums do not match
	 */
	static void validateChecksum(int expectedChecksum, MyByteBuffer data, int offset, int length)
	{
		final int actualChecksum = calculateChecksum(data, offset, length);
		if (actualChecksum != expectedChecksum)
		{
			throw new IllegalArgumentException("mismatching checksum: " + Integer.toHexString(actualChecksum)
	 ...
	 * From the spec:
	 *
	 * "Checksums are not stored directly, but masked, as checksumming data and then its own checksum can be
	 * problematic. The masking is the same as used in Apache Hadoop: Rotate the checksum by 15 bits, then add the
	 * constant 0xa282ead8 (using wraparound as normal for unsigned integers)."
	 *
