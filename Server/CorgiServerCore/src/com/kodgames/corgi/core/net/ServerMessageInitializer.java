...
public class ServerMessageInitializer extends AbstractMessageInitializer
{
	...
	@SuppressWarnings("rawtypes")
	public ServerMessageInitializer(List<String> actionPackageList, List<Class> protocolClassList)
	{
		setBeforeExecutor(new OrderedThreadPoolExecutor(8, 32, 60, TimeUnit.SECONDS, "ServerMessageInitializer"));
		...
		try
		{
			for (Field field : fields)
			{
				...
				if (protocolAnnotation != null)
					...
					logger.warn("Don't set corresponding protocol class for the protocol:{} in the PrococolDesc.xml",
						protocolName);
			}
		}
		catch (Exception e)
	...
	private void bindActionServiceHandler(String packageName) throws Exception
	{
		try
		{
			...
			for (Class actionClass : actionClasses)
			{
				...
				if (annotation != null)
				{
					...
					setHandler(actionAnnotation.messageClass(),
						actionAnnotation.serviceClass(),
						(BaseMessageHandler<?>)actionClass.newInstance());
				}
