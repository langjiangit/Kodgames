package com.kodgames.corgi.core.net.haproxy;
...
public class HAProxyDecoder extends ByteToMessageDecoder {
    private static final Logger logger = LoggerFactory.getLogger(HAProxyDecoder.class);
    ...
    /**
     * {@link ProtocolDetectionResult} for {@link HAProxyDecoder#}.
     */
     ...
     */
    public HAProxyDecoder(int maxTlvSize) {
        if (maxTlvSize < 1) {
    ...
    protected final void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
    	finished = true;
        ProtocolDetectionResult<HAProxyProtocolVersion> result = detectProtocol(in);
        if (result.state().equals(ProtocolDetectionState.INVALID) || result.state().
                equals(ProtocolDetectionState.NEEDS_MORE_DATA)) {
            in.retain();
            out.add(in); //直接转发
            logger.debug("HAProxyDecoder.decode return len {}, {}, {}", in.writerIndex(), in.readerIndex(), in.writerIndex() - in.readerIndex());
            return;
        }
        ByteBuf decoded = null;

        if (result.detectedProtocol().equals(HAProxyProtocolVersion.V1)) {
        	logger.debug("HAProxyDecoder.decode return HAProxyProtocolVersion.V1");
            decoded = decodeLine(ctx, in);
        } else if (result.detectedProtocol().equals(HAProxyProtocolVersion.V2)) {
        	logger.debug("HAProxyDecoder.decode return HAProxyProtocolVersion.V2");
        ...
        if (decoded != null) {
            try {
                ...
                HAProxyMessage msg = (HAProxyMessage) out.get(0);
                HAProxyMessage haProxyMessage = (HAProxyMessage) msg;
                String hostname = haProxyMessage.sourceAddress();
                int port = haProxyMessage.sourcePort();
                ...
                remoteNode.setAddress(remoteAddress);
                logger.debug("HAProxyDecoder.decode set remoteAddress {} {}", hostname, port);
                //logger.debug("HAProxyMessage set {}, HA Server ip {}", hostname, ctx.channel().remoteAddress());
            } catch (HAProxyProtocolException e) {
                //HAProxyMessage exceptionInstance = HAProxyMessage.decodeHeaderNull(result.detectedProtocol());//发生异常的返回对象
                out.add(HAProxyMessage.decodeHeaderNull(result.detectedProtocol()));
                //fail(ctx, null, e);
            }
