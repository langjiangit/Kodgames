package com.kodgames.game.net.server;
		...
		if (connection.getConnectionType() == Connection.CONNECTION_TYPE_SERVER)
		{
			...
			switch (ServerType.getType(serverID))
			{
				...
				case ServerType.BATTLE_SERVER:
					ServiceContainer.getInstance().getPublicService(ServerService.class).onBattleDisconnect(connection);
					break;
				case ServerType.CLUB_SERVER:
					ServiceContainer.getInstance().getPublicService(ServerService.class).onClubDisconnect();
					break;
			}
		}
	}
